  ...
  return (
    ...
    >
      <div className={styles.cardTags}>
        {orderedTags.map((t) => (
          <span key={t} className={`${styles.tag} ${tagClass(t)}`}>{t}</span>
        ))}
      </div>
      <h4 className={`${styles.cardTitle} ${dense ? styles.cardTitleDense : ''}`}>{card.id} - {card.titulo}</h4>
      <div className={styles.cardMeta}>
        <span className={styles.cardMetaLeft}>
          <CheckCircle2 size={16} /> {card.puntos} pts
          <span className="ml-2"><Avatar initials={card.owner} /></span>
        </span>
  ...
  return (
    <div className="flex h-full flex-col gap-5">
      <div className={styles.columnHeader}> 
        ...
        <div className="flex items-center gap-2">
          <span className={styles.columnCount}>{column.cards?.length ?? 0}</span>
          <button
          ...
          <div className={styles.form}>
            <div className="space-y-3">
              ...
              <div className={styles.formRow}>
                ...
                <Button variant="light" onClick={() => setShowForm(false)} className={`h-9 px-3 whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-600`}>Cancelar</Button>
              </div>
            </div>
          </div>
        )}
    ...
    <div className={styles.sprintBoardRoot}>
      <div className={styles.container}>
          ...
          <div className={styles.headerRow}>
            <div className={styles.headerLeft}>
              <div className={styles.headerTitle}>
                <h1 className="text-[clamp(28px,3.4vw,36px)] leading-[clamp(32px,3.8vw,42px)] font-extrabold tracking-tight">Sprint Board</h1>
            ...
            <div className={styles.actions}>
              <div className={styles.avatars}>
                {members.map(m => (
                  <Avatar key={m.id} initials={m.iniciales} color={m.color} className={styles.avatar} />
                ))}
        ...
        <div className={styles.statsGrid}>
          <div className={styles.statCard}>
            ...
            <div className={styles.statValueRow}>
              ...
              <div className={styles.statIconWrap}><Timer size={16} /></div>
            </div>
          </div>
          <div className={styles.statCard}>
            <div className="mb-2">
              <div className="text-sm text-gray-500">Puntos del Sprint</div>
              <div className={styles.statValue}>{stats.puntosHechos}/{stats.puntosTotales}</div>
            </div>
        ...
        <div className={styles.relative}>
          {canLeft && (
            <button type="button" className={styles.leftButton} onClick={() => scrollByDir('left')}>‹</button>
          )}
          {canLeft && <div className={styles.leftShadow} />}
          {canRight && (
            <button type="button" className={styles.rightButton} onClick={() => scrollByDir('right')}>›</button>
          )}
          {canRight && <div className={styles.rightShadow} />}
          <div ref={scrollerRef} className={styles.scroller} onDragOver={handleDragOverScroll}>
